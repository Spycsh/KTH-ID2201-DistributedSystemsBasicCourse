...
% let N server(handler) processes listen to one port
init(Port, N) ->
...
% create a handler pool with N handler processes
handlers(Listen, N) ->
    case N of
	0 ->
	    ok;
...
% each handler process will listen and wait for request
% once after it receives and replies, it is released to the pool 
% and listen and wait for request again
handler(Listen, I) ->
